...
The solar flux at Earth's orbit (outside of the atmosphere) is accepted to be 1337 W/$m^2$ and is referred to as the extraterrestrial solar constant. This can also roughly be calculated if you assume the Sun emits light as a blackbody (which it approximates fairly closely at 5777K) and the diameter of the Sun ($d_{sun}=1.39*10^{9}$ m) as well as the distance between the Earth and Sun ($d_{Earth-Sun}=1.5*10^{11}$ m) are known:
